...
:: ========================================================================================
:: build_env.bat -r requirements.txt -dir C:\Git\CPSSApiGenerator\Dev\CPSSApiGenerator env
:: build_env.bat -r requirements.txt
::
...
set TMP="%HOMEDRIVE%\tmp"
set TEMP="%HOMEDRIVE%\temp"


:defaults
    set "venvwrapper.original_args=%*"
    set "venvwrapper.default_workon_home=%~dp0"
    set "venvwrapper.scriptsdir=Scripts"
    set "venvwrapper.default_env_name=env"
    set "venvwrapper.default_requirements_file=%venvwrapper.default_workon_home%\requirements.txt"
	set "venvwrapper.default_dependency_links_file=%venvwrapper.default_workon_home%\dependency_links.txt"
...
(endlocal & rem export from setlocal block
	...
	set "venvwrapper.dependency_links_file=%dependency_links_file%"
    if  "%venvwrapper.requirements_file%" == "" set "venvwrapper.requirements_file=%venvwrapper.default_requirements_file%"
	if  "%venvwrapper.dependency_links_file%" == "" set "venvwrapper.dependency_links_file=%venvwrapper.default_dependency_links_file%"

    set "venvwrapper.install_packages=%install_packages%"
    set "venvwrapper.virtualenv_args=%venvargs%"
    set "venvwrapper.envname=%envname%"
    if  "%venvwrapper.envname%" == "" set "venvwrapper.envname=%venvwrapper.default_env_name%"
    set "venvwrapper.quoteless_envname=%qlenvname%"
    if  "%venvwrapper.quoteless_envname%" == "" set "venvwrapper.quoteless_envname=%venvwrapper.default_env_name%"
    set "venvwrapper.stop=%stop%"
    set /a venvwrapper.debug=%debug%
)

echo Creating virtualenv named: %venvwrapper.quoteless_envname%
echo at %venvwrapper.default_workon_home%
echo with requirements file %venvwrapper.requirements_file%
echo with requirements file %venvwrapper.dependency_links_file%

pip freeze | findstr /I "virtualenv"
if not errorlevel 1 (
   echo found virtualenv
) else (
    echo cannot find virtualenv, installing...
...
if exist "%WORKON_HOME%\%venvwrapper.quoteless_envname%" (
    echo %WORKON_HOME%\%venvwrapper.quoteless_envname% exists, deleting...
	rmdir /s /q "%WORKON_HOME%\%venvwrapper.quoteless_envname%"
)
...
:: handle -dl
if not "%venvwrapper.dependency_links_file%"=="" (
    if exist "%venvwrapper.dependency_links_file%" (
        echo installing external dependencies
        call "%VIRTUAL_ENV%\Scripts\pip" install -r "%venvwrapper.dependency_links_file%"
    ) else (
        echo dependency_links_file does not exists.
    )
)
